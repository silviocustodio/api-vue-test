            ...
            <div class="movie__details">
              ...
              <div v-if="movie.release_date" class="movie__details-block">
                ...
                <div
                  ...
                  v-formatDate="movie.release_date"
                ></div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    fetchMovie(id) {
      axios
        .get(
          `https://api.themoviedb.org/3/movie/${id}?api_key=${storage.apiKey}&language=en-US`
        )
        .then(
          function (resp) {
            let movie = resp.data;
    ...
    checkIfInFavorites(id) {
      axios
        .get(
          `https://api.themoviedb.org/3/movie/${id}/account_states?api_key=${storage.apiKey}&session_id=${storage.sessionId}`
        )
        .then(
          function (resp) {
    ...
    toggleFavorite() {
      ...
      axios
        .post(
          `https://api.themoviedb.org/3/account/${storage.userId}/favorite?api_key=${storage.apiKey}&session_id=${storage.sessionId}`,
          {
            media_type: "movie",
            media_id: this.id,
